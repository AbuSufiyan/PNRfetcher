...
public class Test {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length > 0) {
			...
			passengerDetails.append("<h2>PNR is empty.</h2></br> sample: java -jar Test 4810991753");
			System.out.println("PNR missing. sample: java -jar Test 4810991753.");
		}
		...
		if ("Linux".equalsIgnoreCase(System.getProperty("os.name"))) {
			// ProcessBuilder pBuilder = new ProcessBuilder("cmd.exe", "/c", "start " + "Status.html");
			// pBuilder.redirectErrorStream(true);
			// Process startDirectory = pBuilder.start();

			Runtime runtime = Runtime.getRuntime();
			try {
				runtime.exec("xdg-open " + "Status.html");
			} catch (IOException e) {
	...
	private static String[] checkIfFileSuppliedForPNR(String[] args) throws IOException {
		if (args.length == 1) {
	...
	private static StringBuilder FetchDetails(String body, String PNR, boolean firstEntry) throws Exception {
		...
		if (body == null) {
			...
			passengerDetails.append("</br></br><b style=\"color:red\">Incorrect PNR : " + PNR + "</b>");
	...
	private static String getTrainDetails(Elements trainElements) {
		...
		for (Element element : trainElements) {
			...
			switch (i) {
				case 0:
					trainDetails.append("" + element.text());
					i++;
					break;
				case 1:
					trainDetails.append(" - " + element.text());
	...
	private static String FetchCurrentStatus(String pnr) throws IOException {
		URL url = new URL("http://www.indianrail.gov.in/cgi_bin/inet_pnstat_cgi_28688.cgi");
		url = new URL("http://www.indianrail.gov.in/cgi_bin/inet_pnstat_cgi_28688.cgi");
		HttpURLConnection con = (HttpURLConnection) url.openConnection();
		con.setDoOutput(true);
		// con.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
		con.setRequestProperty("Accept-Encoding", "gzip, deflate");
		con.setRequestProperty("Referer", "http://www.indianrail.gov.in/pnr_Enq.html");

		String urlParams = "lccp_cap_val=1234&lccp_capinp_val=1234&lccp_pnrno1=" + pnr + "&submit=Get+Status";
		// 4810991753
		...
		try {
			byte[] buffer = new byte[6553];
			int noRead;

			while ((noRead = in.read(buffer)) > 0) {
		...
		// String gh = IOUtils.toString(in, encoding);
		// in.close();
		return sb.toString();
	}

	private static StringBuilder DisplayDetails(String data, String PNR, boolean firstEntry, String trainDetails)
	    throws Exception {
		String result = data.substring(data.indexOf("Passenger"));
		String passenegerStatusString = result.substring(0, result.indexOf("Charting Status"));
		StringBuilder sb = new StringBuilder();
